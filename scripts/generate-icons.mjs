...
// SDF-style point sampling for the brand mark
const BRAND = {
...
function renderMark(size, opts = {}) {
  ...
  // Stroke radii for "A"
  const strokeR = 2.5 * scale; // half of stroke-width 5

  // "A" geometry: legs from (16,47) to (32,17) and (32,17) to (48,47); crossbar (22.5,37.5)→(41.5,37.5)
  const segs = [
    [16, 47, 32, 17],
    [32, 17, 48, 47],
    [22.5, 37.5, 41.5, 37.5],
  ...
  // Accent dot
  const dotX = ox + 46 * scale;
  const dotY = oy + 20 * scale;
  const dotR = 4 * scale;
  ...
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      ...
      if (dotAlpha > 0) {
        rgb = blend(rgb, BRAND.accent, dotAlpha);
...
function renderOgImage(width, height) {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      const ty = y / height;
      // Dark gradient
      const rgb = mix([15, 23, 42], [30, 58, 138], (tx + ty) / 2);
      buf[i] = rgb[0];
      buf[i + 1] = rgb[1];
      buf[i + 2] = rgb[2];
      buf[i + 3] = 255;
